 ...
 */
public final class TrainReader {

    private enum TrainDataHeader {
        user,event,invited,timestamp,interested,not_interested
    }

    private static final Logger logger  = LoggerFactory.getLogger(TrainReader.class);

    public static final void readTrainData(String fileName) {
        ...
        try {
            ...
            BufferedWriter writer = new BufferedWriter(new FileWriter("/Users/ravirajmulasa/Documents/Certificate_in_MachineLearning/Advanced_Machine_Learning/RecommendationEngine/models/train_feature.csv"));
...
//            printer.print(TestFeatureHeader.user_age_in_days);
            ...
            printer.print("not_interested");
            printer.println();
            printer.flush();

            for (CSVRecord record : records) {

                Long userId         = Util.readValue(record.get(TrainDataHeader.user).trim(), -1L);
                Long eventId        = Util.readValue(record.get(TrainDataHeader.event).trim(),-1L);

                int invited         = Util.readValue(record.get(TrainDataHeader.invited).trim(), 0);
                String timeUsrSaw   = Util.readValue(record.get(TrainDataHeader.timestamp).trim(), "2012-11-16T16:01:55.620Z");

                int interested          = Util.readValue(record.get(TrainDataHeader.invited).trim(), 0);
                int notInterested         = Util.readValue(record.get(TrainDataHeader.not_interested).trim(), 0);
